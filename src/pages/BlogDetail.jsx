...
const BlogDetail = () => {
  ...
  if (loading) {
    return <div className="min-h-screen bg-[#f6faf7] p-8 text-center text-sm font-semibold text-slate-500">Loading article…</div>
  }
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-[#f6faf7] px-4">
        <div className="max-w-md rounded-2xl bg-white p-8 text-center shadow-sm">
          <h1 className="text-2xl font-semibold text-slate-950">Blog post not found</h1>
          <p className="mt-3 text-sm leading-7 text-slate-600">This CMS blog entry is not available or has been unpublished.</p>
          <Link to="/blog" className="mt-6 inline-flex rounded-xl bg-slate-950 px-5 py-3 text-sm font-semibold text-white">Back to blog</Link>
        </div>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-[#f6faf7] text-slate-950">
      <main>
        <article className="mx-auto max-w-4xl px-4 py-10 sm:px-6 lg:px-8">
          <Link to="/blog" className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 shadow-sm hover:bg-slate-50">
          ...
          <header className="mt-10">
            ...
            <h1 className="mt-6 text-4xl font-semibold leading-tight tracking-tight text-slate-950 sm:text-6xl">{title}</h1>
          ...
          <section className="mt-10 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
            ...
            <h2 className="mt-10 text-2xl font-semibold text-slate-950">Complete guide</h2>
            <div className="mt-4 space-y-5 text-base leading-8 text-slate-700">
              {(paragraphs.length ? paragraphs : [getPostExcerpt(post)]).map((paragraph) => (
                <p key={paragraph}>{paragraph}</p>
              ))}
            </div>

            <h2 className="mt-10 text-2xl font-semibold text-slate-950">Next action</h2>
