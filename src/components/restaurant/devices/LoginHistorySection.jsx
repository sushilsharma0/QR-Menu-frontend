...
export default function LoginHistorySection({ history = [], loading }) {
  const rows = history.slice(0, HISTORY_LIMIT)

  return (
    <motion.section {...sectionMotionDelayed(0.2)}>
                ...
                {rows.length ? (
                  rows.map((item, index) => (
                    ...
                    >
                      ...
                      <td className="px-3 py-3">{formatDeviceLocation(item.loginLocation)}</td>
                      <td className="px-3 py-3">{formatDeviceDate(item.createdAt)}</td>
                      <td className="px-3 py-3">
                        {item.revokedAt ? (
