...
export default function PasswordChangeReminderModal() {
  ...
  const reasonText = useMemo(() => {
    if (recommendation.reason === 'age') {
      return `Your password is ${recommendation.passwordAgeDays || 0} days old.`
    }
    if (recommendation.reason === 'logins') {
      return `This password has been used for ${recommendation.successfulLoginCount || 0} successful login(s).`
    }
    return 'It is a good time to refresh your restaurant password.'
  }, [recommendation.passwordAgeDays, recommendation.reason, recommendation.successfulLoginCount])
  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-2xl border border-gray-200 bg-white p-6 shadow-2xl dark:border-gray-800 dark:bg-gray-900">
        ...
        <h2 className="mt-4 text-xl font-semibold text-gray-950 dark:text-gray-100">Update your password</h2>
        <p className="mt-2 text-sm leading-6 text-gray-600 dark:text-gray-400">
          {reasonText} Changing it now helps protect orders, billing, staff access, and restaurant settings.
        </p>
        ...
        <div className="mt-4 flex items-start gap-3 rounded-lg bg-primary-50 px-3 py-3 text-sm text-primary-800 dark:bg-gray-800 dark:text-primary-200">
          ...
          <p>
            We recommend changing restaurant passwords every {recommendation.reminderAfterDays || 90} days or after{' '}
            {recommendation.reminderAfterLogins || 10} successful login(s).
          </p>
        </div>
