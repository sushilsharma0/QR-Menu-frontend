...
const BlogCard = ({ post, index = 0 }) => (
  <motion.article
    ...
    className="group overflow-hidden rounded-2xl border border-surface-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl"
  >
    <Link to={getPostUrl(post)} className="block" aria-label={`Learn more about ${getPostTitle(post)}`}>
      <div className="relative overflow-hidden">
        ...
        <span className="absolute left-4 top-4 inline-flex items-center gap-2 rounded-full bg-white/90 px-3 py-2 text-xs font-black text-primary-700 shadow-sm backdrop-blur">
